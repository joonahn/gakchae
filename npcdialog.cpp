...
void Npcdialog::insol1Dialog(int index)
{
    ...
    else if(index==3)
    {
        ...
        dialogtextlabel->setText("훌륭하군! 내 레포트를 찾아준 보답으로 육일콜 번호를 주도록 하지, 아 그리고 밖에 나갈거면 학생증이 필요할거야. 나는 없는데 학생증 가지고 있는 다른 인솔자 방 비밀번호를 알려줄게. " + QString::number(map->getPasswd()) + "잘 외워 두라고!");
    ...
    else if(index==6)
    {
        ...
        delete pixmap;
        //isInsol2Visited() == false
        dialogtextlabel->setText("인솔자 1은 피곤한 것처럼 보인다.");
        dialogselectlabel[0]->setText("▶나가기");
        dialogselectlabel[1]->setText("");
        currentchoice = 0;
        nselect = 1;
...
void Npcdialog::insol2DIalog(int index)
{
    ...
    else if(index==4)
    {
        ...
        dialoglabel->setPixmap(*pixmap);
        delete pixmap;
        dialogtextlabel->setText("인솔자 2는 피곤한 듯 보인다.");
        dialogselectlabel[0]->setText("▶대화 끝내기");
        dialogselectlabel[1]->setText("");
        currentchoice = 0;
        nselect = 1;
...
void Npcdialog::friendDialog(int index)
{
    ...
    if(index==1)
    {
        ...
        dialogselectlabel[0]->setText("▶나가기");
        dialogselectlabel[1]->setText("흔들어 깨운다");
        nselect = 2;
        currentchoice = 0;
    }
    else if(index==2)
    {
        ...
        dialogselectlabel[1]->setText("아 아무일도 아니야");
        currentchoice = 0;
        nselect = 2;
    }
    else if(index==3)
    ...
    {
        dialogtextlabel->setText("아, 잘 자고 있었는데 누구야...!");
        dialogselectlabel[0]->setText("▶미안하다 하고 방을 나간다");
        dialogselectlabel[1]->setText("나가자고 꼬신다");
        currentchoice = 0;
        nselect = 2;
    }
    currentindex = index;
